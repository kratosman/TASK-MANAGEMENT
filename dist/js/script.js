        ...
        if (boardsTaskStorage && boardsTaskStorage[currentIndex].task_log.length > 0) {
                ...
                section.forEach((sectionItem) => {
                    ...
                    for (let ins = 0; ins < sectionItem.children.length; ins++) {
                        (function(ins) {
                            sectionItem.children[ins].addEventListener('click', (event) => {
                                var clickedTask = event.currentTarget;
                                console.log(clickedTask)
                                var dataIndex = Array.from(sectionItem.children).indexOf(clickedTask);
                                var dataList = boardsTaskStorage[currentIndex].task_log[dataIndex].subtask.map(itemList => itemList.statusList);
                        var taskDetailModal = document.createElement('div');
                            ...
                            taskDetailModal.innerHTML += `<!-- TASK CONTENT -->
                                ...
                                <div class="task-descriptions">
                                    ${boardsTaskStorage[currentIndex].task_log.description}
                                </div>
                                <div class="subtask-container">
                                    <span><h4>Subtask ( 0 of ${item.subtask.length})</h4></span>
